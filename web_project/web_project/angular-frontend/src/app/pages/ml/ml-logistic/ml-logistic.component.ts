...
@Component({
  selector: 'app-ml-logistic',
  standalone: true,
  imports: [RouterLink, FormsModule, LoadingSpinnerComponent, ErrorMessageComponent],
  templateUrl: './ml-logistic.component.html',
  styleUrls: ['./ml-logistic.component.css']
})
export class MlLogisticComponent {
  xValues = '[[1, 2], [2, 3], [3, 1], [4, 3]]';
  yValues = '0, 0, 1, 1';
  predictValue = '[2, 2]';
  ...
  onSubmit() {
    ...
    if (!Array.isArray(x) || x.some(row => !Array.isArray(row))) {
      this.isLoading = false;
      this.errorMessage = 'La matriz X debe ser un arreglo bidimensional (ej. [[1, 2], [3, 4]]).';
      return;
    }

    if (y.some(isNaN)) {
      this.isLoading = false;
      this.errorMessage = 'El vector Y contiene etiquetas no numéricas.';
      return;
    }
    ...
    this.mlService.runLogistic(payload).subscribe({
      next: (res) => {
        this.result = res;
        this.isLoading = false;
      },
      error: (err) => {
        this.isLoading = false;
        this.errorMessage = 'Error al ejecutar regresión logística: ' + err.message;
      }
    });
  }
